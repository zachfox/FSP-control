    ...
    def _run_trajectory(self):#!!!!!!!!!!!!!!!!!renamed run to solve(big deal)
        ...
        if self.type=='linear':
            ...
            while t<self.tf:
                ...
                rate = np.concatenate((np.array([self.fast_rxn]),trate.ravel()))
                rate=np.cumsum(rate)
                t=(t-np.log(np.random.rand(1))/rate[-1])
                ro=rate[-1]*np.random.rand()
                while t>self.tvec[ip]:
                    if t>self.tf:
                        b = len(self.tvec[ip:])
...
#                print '****USING EXPV****'
#                for i in range(1,len(self.tvec)):
#                    pv,m,v = expv(self.tvec[i]-self.tvec[i-1],self.A,pv,tol = tol,m=15)
#                    self.soln[:,i] = pv 
#        #           self.errors[i]=1.0-np.sum(self.model_solns[:,i])
#            except:
#                print 'EXPV failed, integrating' 
#                self.ODE = lambda x,t: np.dot(self.A,np.array([x]).T).ravel()
#                solution=odeint(self.ODE,self.pi,self.tvec,mxstep = 100000)       
#                self.soln = solution.T
#        else:
#            print '****USING EXPM****'
#            for i in range(1,len(self.tvec)):
#               
#               pv = expm(self.A*(self.tvec[i]-self.tvec[i-1])).dot(pv)
#               self.soln[:,i] = pv 
...
class GenericODE:
    ...
    def _solve(self):
        ...
        '''
        soln = solve_ivp(self.ODE, (0,self.tvec[-1]), self.xi, method='BDF', t_eval=self.tvec )
        self.soln = soln.y
        return self.soln
        # self.tvec=self.gettvec()
        # nexp_max = 5
...
# #            print("tvec now has %d points" %len(tvec_for_integration))
# #            solution=odeint(self.ODE,self.xi,tvec_for_integration,mxstep = 100000)       
# #            nexp+=1
# #            solution=odeint(self.ODE,self.xi,tvec_for_integration,mxstep = 100000,Dfun:w
# =self.jacobian)       

#             try: 
#                 #solution=self.odeint_new(self.ODE,self.xi,tvec_for_integration)               
#                 try: 
...
# #                traceback.print_exc()
#                 print( "integration failed! increaasing time points")
#                 nexp += 1
...
#         return -1
    ...
    def odeint_new(self,func, y0, t, Dfun=None):
        '''
        make easier interface for odeint
        WIP
        '''
        ig = ode(func)
        ig.set_integrator('vode',method='bdf')
...
class GenericMoments(GenericODE):
    ...
    models with affine-lienar propensities.
    '''
    ...
    def get_mean(self):
        '''
        get the 1st moment of the stochastic system
        '''
    ...
    def get_moments(self,order=1,ss=False):
        '''
        Get specified moments, up to second moment.
        ''' 
        if order == 1:
            if ss:
...
#        uu[0,0] = 1.0
    ...
    def sensitivity_ODE(self,t,x):
        ...
        if not self.tv:
            inner = np.dot(self.W1,np.array([_MU]).T)+self.W0(t)
#            tmp =  self.K_subs_i.subs(self.species_dict)
#            K_subs = np.ravel(np.array(tmp.tolist()).astype(np.float64))
        ...
        # Compute RHS
        RHS_vars = np.ravel(np.dot(np.dot(self.S,self.W1),_SIG) + np.dot(np.dot(_SIG,self.W1.T),self.S.T) + np.dot(np.dot(self.S,self.diag_mat),self.S.T))
        RHS_means = np.ravel(np.dot(self.S,(np.dot(self.W1,np.atleast_2d(_MU).T)))+np.dot(self.S,self.W0(t)))
    ...
    def get_FIM(self,order=1,tstart=0,log=False):
        ...
        #self.xi = np.concatenate((self.xi,np.zeros(self.N+2*self.N**2)))
        # self.get_S_sym()
        # self.get_W_sym()
...
#            print(K_subs)
            ...
            tmp = np.reshape(self.soln[s_v:,tstart:],(self.N,self.N,ntimes))[self.observables,:,:][:,self.observables,:]
            ...
            for j in range(ntimes):
                self.dv_dtheta[j*self.N_observables:j*self.N_observables+self.N_observables,j*self.N_observables:j*self.N_observables+self.N_observables,i] =tmp[:,:,j]
...
#        self.dm_dtheta = self.dm_dtheta[:,self.params_to_keep]
        ...
        for t in range(ntimes):
            ...
            tmp =  np.reshape(self.soln[self.N:self.N+self.N**2,tstart+t],(self.N,self.N))[self.observables,:][:,self.observables]
            # tmp2 = np.array([tmp[self.observables,:]])
            # tmp3 = np.array([tmp2[:,self.observables]])
            self.covariances[:,:,t] = tmp
