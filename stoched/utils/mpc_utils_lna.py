...
class StateEstimation:
    ...
    def predict_state(self, x, P, model, dt):
        ...
        soln = model.solve()
        return soln.mean[:,-1], soln.covariances[:,:,-1]/self.ncells
    ...
    def estimate_state(self, x, P, yk):
        ...
        d = x.shape[0]
        K = np.dot( P, self.C.T).dot(np.linalg.pinv(np.dot(self.C, P).dot(self.C.T)+self.R)) # changed this
...
class MPC(StateEstimation):
    def __init__(self, model, period, C, R, horizon=None, full_target=None):
        ...
        self.full_control_vector[1] = 1
        self.model = model
        self.x_est = self.model.params.x0[:self.model.N]
        self.P_est = self.model.params.x0[self.model.N:].reshape(self.model.N,self.model.N)
        self.iteration = 0
        self.all_pk = []
    ...
    def optimize_binary_control(self, target, x, P):
        ...
        self.model.params.x0 = np.concatenate((x,P.ravel()))
    ...
    def cost_function(self, target, control_vector, model, period):
        ...
        sol = model.solve()
        mu = np.dot(self.C,sol.mean)
       # sig = np.dot(self.C,sol.covariances)
        return np.sum(((target*self.C[0,2]-mu)**2))
