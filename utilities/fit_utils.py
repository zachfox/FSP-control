...
def train_crossval( model, data_collection, n_repeats=1):
    ...
    for i in range(n_repeats):
        ...
        results = model.fit(dc_train, optimizer='nelder_mead')
...
def train_model_collection( model_collection, data, fit_args):
    '''
    Train all the models in the collection of models to the 
    data object
    '''
    all_results = []
    for model in model_collection:
        results = model.fit(data, **fit_args)
#        results = model.fit(data, optimizer='nelder_mead')
        all_results.append(results)
    return all_results
